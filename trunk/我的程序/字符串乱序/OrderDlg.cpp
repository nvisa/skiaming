...
void COrderDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void COrderDlg::OnBtnBegin() 
{
	// TODO: Add your control notification handler code here
	CString sContent;
	GetDlgItemText(IDC_EDIT_CONTENT, sContent);
	int iCount = sContent.GetLength();
	CString sFileName;
	GetDlgItemText(IDC_EDIT_FILENAME, sFileName);
	sFileName += ".txt";
	int iDestNumber = GetDlgItemInt(IDC_EDIT_NUMBER);
	CFile file;
	if(file.Open(sFileName, CFile::modeCreate | CFile::modeWrite))
	{
		file.SeekToBegin();
		file.Write("\xff\xfe", 2);
		srand(time(0));	//随机器种子必须放在外面
		for(int i = 0; i != iDestNumber; ++i)
		{		
			TCHAR sTemp = sContent.GetAt(rand()%iCount);
			file.SeekToEnd();
			file.Write(&sTemp, sizeof(sTemp));
		}
		file.Close();
	}
